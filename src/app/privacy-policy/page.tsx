...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const doc: any = documents.find((d: any) => d.slug === "privacy-policy" && d.status === "published");

  const title = doc?.metaTitle || doc?.title || "Privacy Policy";
  const description =
    doc?.metaDescription ||
    "Read The Vertical AI's Privacy Policy to understand how we collect, use, and protect your data.";
...
export default async function PrivacyPolicy() {
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <PageHero
        title={title}
        backgroundImage={doc?.bannerUrl || "https://images.unsplash.com/photo-1526374965328-7f61d4dc18c5?q=80&w=2940&auto=format&fit=crop"}
      />
      ...
      <main className="max-w-4xl mx-auto px-6 pt-16 pb-24">
        ...
        <div className="mt-12 p-6 bg-blue-50 rounded-2xl border border-blue-100">
          ...
          <p className="text-blue-800">
            Reach out to our privacy team at <a href={`mailto:${supportEmail}`} className="font-medium underline hover:text-blue-600 transition-colors">{supportEmail}</a>.
          </p>
        </div>
      </main>
